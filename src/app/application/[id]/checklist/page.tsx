...
import {
  Rocket, ArrowLeft, CheckCircle, Circle, AlertTriangle,
  FileCheck, Brain, DollarSign, Settings, ClipboardCheck, ChevronDown, ChevronRight,
  ExternalLink
} from 'lucide-react'
...
export default function ChecklistPage() {
    ...
    if (session && appId) {
      fetch('/api/applications')
        ...
        .then((apps) => {
          ...
          if (app) {
            setApplication(app)
            setCheckedItems(app.checklistData || {})
          }
        })
    }
  }, [session, appId])
  ...
  const categoryIcons: Record<string, any> = {
    ...
    'SIN Selection': ClipboardCheck,
    'Technical Proposal': FileCheck,
    Pricing: DollarSign,
        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="h-16 flex items-center justify-between">
            <div className="flex items-center gap-4">
              <button onClick={() => router.push('/dashboard')} className="text-slate-500 hover:text-white">
        ...
        <div className="card mb-8">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-sm font-semibold text-slate-400">
              {completedItems} / {totalItems} required items
            </span>
          </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <div className="text-xs text-slate-500 mt-0.5">
                        {catCompleted}/{catTotal} complete
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                  ...
                  <div className="border-t border-slate-700/50">
                    {items.map((item) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className={`flex items-start gap-4 p-5 border-b border-slate-700/30 last:border-0 hover:bg-slate-700/10 transition-colors ${
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <span className={`font-medium text-sm ${isChecked ? 'text-slate-400 line-through' : 'text-white'}`}>
                                {item.label}
                              </span>
                              {item.required && !isChecked && (
                                <span className="badge-danger text-[10px]">Required</span>
                              )}
                              {item.estimatedMinutes && !isChecked && (
                                <span className="text-[10px] text-slate-600">~{item.estimatedMinutes}min</span>
                              )}
                            </div>
